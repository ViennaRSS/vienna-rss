...
@property (readwrite, nonatomic) NSString *feedDescription;
@property (readwrite, nonatomic) NSString *link;
@property (readwrite, nonatomic) NSDate *lastModified;
...
@property (nonatomic) NSString *rssPrefix;
@property (nonatomic) NSString *rdfPrefix;
@property (nonatomic) NSString *atomPrefix;
@property (nonatomic) NSString *dcPrefix;
@property (nonatomic) NSString *contentPrefix;
@property (nonatomic) NSString *mediaPrefix;
@property (nonatomic) NSString *encPrefix;
...
- (BOOL)initRSSFeedHeaderWithElement:(NSXMLElement *)channelElement
{
    ...
    for (NSXMLElement *element in channelElement.children) {
        ...
        if (isRSSElement && [channelItemTag isEqualToString:@"link"]) {
            self.link = (element.stringValue).vna_stringByUnescapingExtendedCharacters;
            success = YES;
            continue;
        }
...
- (BOOL)initRSSFeedItems:(NSXMLElement *)startElement
{
    ...
    NSAssert(self.mutableItems == nil, @"initRSSFeedItems called more than once per initialisation");
    ...
    for (NSXMLElement *element in startElement.children) {
        ...
        if ([element.prefix isEqualToString:self.rssPrefix] && [element.localName isEqualToString:@"item"]) {
            ...
            for (NSXMLElement *itemChildElement in element.children) {
                ...
                if ((isRSSElement && [articleItemTag isEqualToString:@"author"]) || ([itemChildElement.prefix isEqualToString:self.dcPrefix] && [articleItemTag isEqualToString:@"creator"])) {
                    ...
                    if (authorName) {
                        ...
                        if (newFeedItem.author.length > 0 && [newFeedItem.author rangeOfString:authorName
            ...
            if (index == NSNotFound || index >= self.mutableItems.count) {
                [self.mutableItems addObject:newFeedItem];
            } else {
                [self.mutableItems insertObject:newFeedItem atIndex:index];
            }
        }
    }
...
- (BOOL)initAtomFeed:(NSXMLElement *)atomElement
{
    ...
    // Allocate an items array
    NSAssert(self.mutableItems == nil, @"initAtomFeed called more than once per initialisation");
    self.mutableItems = [[NSMutableArray alloc] init];
