...
- (void)sendBlogEvent:(NSString *)externalEditorBundleIdentifier title:(NSString *)title url:(NSString *)url body:(NSString *)body
               author:(NSString *)author guid:(NSString *)guid;
...
- (void)doSafeInitialisation
{
    ...
    if (!doneSafeInit) {
        ...
        [self.mainWindow makeFirstResponder:(previousArticleGuid !=
                                             nil) ? [self.browser primaryTabItemView].mainView : self.foldersTree.mainView];
        ...
        for (int i = 0; i < tabLinks.count; i++) {
            ...
            [self.browser createNewTab:([NSURL URLWithString:tabLink]) withTitle:tabTitles[tabLink] inBackground:YES];
        }
...
- (void)installScriptsFolderWatcher {
    ...
    [self.directoryMonitor startAndReturnError:&error eventHandler:^{ [weakSelf initScriptsMenu]; }];
    if (error) {
...
- (BOOL)application:(NSApplication *)theApplication openFile:(NSString *)filename
{
    ...
    if ([filename.pathExtension isEqualToString:@"viennastyle"]) {
        ...
        if (![self installFilename:filename toPath:prefs.stylesFolder]) {
            ...
            runOKAlertPanel(NSLocalizedString(@"Vienna has installed a new style", nil),
                            NSLocalizedString(@"The style \"%@\" has been installed to your Styles folder and added to the Style menu.",
                                              nil), styleName);
        }
        return YES;
    }
    if ([filename.pathExtension isEqualToString:@"viennaplugin"]) {
        ...
        if ([self installFilename:filename toPath:path]) {
            runOKAlertPanel(NSLocalizedString(@"Plugin installed", nil),
                            NSLocalizedString(
                                @"A new plugin has been installed. It is now available from the menu and you can add it to the toolbar.",
                                nil));
...
- (BOOL)installFilename:(NSString *)srcFile toPath:(NSString *)path
{
    ...
    if (![fileManager fileExistsAtPath:path isDirectory:&isDir]) {
        if (![fileManager createDirectoryAtPath:path withIntermediateDirectories:YES attributes:NULL error:NULL]) {
            runOKAlertPanel(NSLocalizedString(@"Cannot create folder", nil), NSLocalizedString(@"The \"%@\" folder cannot be created.",
                                                                                               nil), path);
            return NO;
        }
    }
...
- (void)openURLsInDefaultBrowser:(NSArray<NSURL *> *)urlArray {
    ...
    NSWorkspaceLaunchOptions lOptions =
        openLinksInBackground ? (NSWorkspaceLaunchWithoutActivation | NSWorkspaceLaunchDefault) : (NSWorkspaceLaunchDefault |
...
- (IBAction)exportSubscriptions:(id)sender
{
    ...
    ExportAccessoryViewController *accessoryController =
        [[ExportAccessoryViewController alloc] initWithNibName:@"ExportAccessoryViewController" bundle:nil];
    ...
    [panel beginSheetModalForWindow:self.mainWindow completionHandler:^(NSInteger returnCode) {
        if (returnCode == NSModalResponseOK) {
            ...
            NSInteger countExported =
                [Export exportToFile:panel.URL.path fromFoldersTree:self.foldersTree selection:accessoryController.mode ==
                 ExportModeSelectedFeeds withGroups:accessoryController.preserveFolders];

            if (countExported < 0) {
                NSAlert *alert = [NSAlert new];
                alert.messageText = NSLocalizedString(@"Cannot create export output file", nil);
                alert.informativeText = NSLocalizedString(
                    @"The specified export output file could not be created. Check that it is not locked and no other application is using it.",
                    nil);
                ...
                alert.informativeText =
                    [NSString stringWithFormat:NSLocalizedString(@"%d subscriptions successfully exported", nil), (int)countExported];
                [alert runModal];
            }
        }
    }];
...
- (void)initSortMenu
{
    ...
    NSMenuItem *menuItem =
        [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Ascending", nil) action:@selector(doSortDirection:) keyEquivalent:@""];
    menuItem.representedObject = @YES;
    [sortSubmenu addItem:menuItem];
    menuItem =
        [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Descending", nil) action:@selector(doSortDirection:) keyEquivalent:@""];
...
- (void)initScriptsMenu {
    ...
    if (count > 0) {
        ...
        menuItem =
            [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Open Scripts Folder", nil)
                                       action:@selector(doOpenScriptsFolder:) keyEquivalent:@""];
        [scriptsMenu addItem:menuItem];

        menuItem =
            [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"More Scripts…", nil) action:@selector(moreScripts:) keyEquivalent:@""];
        [scriptsMenu addItem:menuItem];
    ...
    @synchronized(NSApp.dockTile) {
        ...
        if (@available(macOS 11.0, *)) {
            ...
            self.mainWindow.title =
                [NSString stringWithFormat:@"%@ (%li %@)", self.appName, (long)currentCountOfUnread, NSLocalizedString(@"Unread", nil)];
        }
...
- (IBAction)emptyTrash:(id)sender
{
    ...
    [alert addButtonWithTitle:NSLocalizedString(@"Cancel", @"Title of a button on an alert")];
    [alert
     beginSheetModalForWindow:self.mainWindow
    ...
    if (self.connecting) {
        ...
        if (newUnread > 0) {
            ...
            notification.informativeText =
                [NSString stringWithFormat:NSLocalizedString(@"%d new unread articles retrieved", @"Notification body"), (int)newUnread];
        ...
        case NSRightArrowFunctionKey:
            if (flags & (NSEventModifierFlagCommand | NSEventModifierFlagOption)) {
                ...
                if (self.mainWindow.firstResponder == self.foldersTree.mainView) {
                    ...
                    [self.mainWindow makeFirstResponder:(self.selectedArticle !=
                                                         nil) ? [self.browser primaryTabItemView].mainView : self.foldersTree.mainView];
                    return YES;
                }
            }
...
- (IBAction)deleteFolder:(id)sender
{
    ...
    if (count == 1) {
        ...
        alertBody =
            [NSString stringWithFormat:NSLocalizedString(
                 @"Are you sure you want to delete all %d selected folders? This operation cannot be undone.",
                 nil), (unsigned int)count];
...
} // updateSearchPlaceholderAndSearchMethod
...
{
    if (self.mainWindow.toolbar.visible && [self toolbarItemWithIdentifier:@"SearchItem"] &&
        self.mainWindow.toolbar.displayMode != NSToolbarDisplayModeLabelOnly)
    {
...
- (void)blogWithExternalEditor:(NSString *)externalEditorBundleIdentifier
{
    ...
    NSView<BaseView> *theView = self.browser.activeTabItemView;
    if ([theView isKindOfClass:[BrowserPane class]]) {
        [self sendBlogEvent:externalEditorBundleIdentifier title:[self.browser tabItemViewTitle:self.browser.activeTabItemView]
                        url:theView.viewLink body:APP.currentTextSelection author:@"" guid:@""];
    } else {
        // Get the currently selected articles from the ArticleView and iterate over them.
        for (Article *currentArticle in self.articleController.markedArticleRange) {
            [self sendBlogEvent:externalEditorBundleIdentifier title:currentArticle.title url:currentArticle.link body:APP.
             currentTextSelection author:currentArticle.author guid:currentArticle.guid];
        }
    }
} // blogWithExternalEditor
               ...
               author:(NSString *)author guid:(NSString *)guid
{
    ...
    target = [NSAppleEventDescriptor descriptorWithDescriptorType:typeApplicationBundleID
                                                             data:[externalEditorBundleIdentifier dataUsingEncoding:NSUTF8StringEncoding]];
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
    ...
    if (theAction == @selector(printDocument:)) {
        ...
        menuItem.state =
            [styleName isEqualToString:[Preferences standardPreferences].displayStyle] ? NSControlStateValueOn : NSControlStateValueOff;
        return isMainWindowVisible && isAnyArticleView;
...
/**
 * Toggle the visibility of the activity panel; show when hidden and close when visible.
 */
