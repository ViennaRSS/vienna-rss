				   ...
				   defer:(BOOL)flag
{
	...
	if ((self = [super initWithContentRect:contentRect styleMask:newStyle backing:bufferingType defer:flag]) != nil)
	{
		...
		// Cache the graphics
		bottomMiddle = [[NSImage imageNamed:@"bottom_middle"] retain];
		topLeft = [[NSImage imageNamed:@"top_left"] retain];
		topMiddle = [[NSImage imageNamed:@"top_middle"] retain];
		topRight = [[NSImage imageNamed:@"top_right"] retain];

		// Find background color to draw into window
		[topMiddle lockFocus];
		bgColor = [NSReadPixel(NSMakePoint(0, 0)) retain];
